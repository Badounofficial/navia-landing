# Navia — Building the Intelligent Companion App
...
An earlier version of this file proposed a React Native + Supabase stack. Since then, four foundation documents have been written in `Navia App/navia-companion/`:

- `personality.md` — who Navia is, her voice, her values, her limits
...
5. **Voice.** Custom Navia voice, recorded with a real voice actress, synthesized through a provider that supports voice cloning with strict consent.
...
| Voice synthesis | **Custom Navia voice** via a provider supporting voice cloning | See `architecture.md` §4 for the v1 approach. |
...
| Phase 4 — The voice | August → September 2026 | Voice actress recorded, custom Navia voice integrated, pre-rendered phrases cached on device. |
...
Dates are targets, not commitments. Navia launches when Navia is ready, not before.
...
- Two showcase sections ("A day with Navia" and "Inside Navia") shipping Navia's visual language.
- Hero phone with a multi-turn conversation that demonstrates Navia's voice.
- Companion personality, system prompt v1, architecture, and MVP feature scope drafted.

**To do before we leave Phase 0:**
- [ ] Consult a US IP attorney on the "Navia" trademark, given adjacent "Navia Benefit Solutions" (health benefits) already exists. (See `project_legal_reminder.md`.)
...
Build the real Navia app skeleton. No AI yet, no backend yet, no voice yet. Everything runs on device.

**Screens to build, in this order:**

1. **Welcome.** Brand, logo, one line, "Get started" button.
2. **Onboarding.** Four soft questions: name, age, chapter, one thing Navia should know about today.
...
6. **Cycle.** Cycle ring view (same visual language as the landing page "Inside Navia" card), period logging.
7. **Pregnancy.** Week-by-week view, activated only if the user said she is pregnant.
8. **Settings.** Notification controls, export all data, "forget everything", Navia's "Navia knows" memory sheet, founder page.
...
Wire the companion model behind a thin, privacy-first API layer. This is where Navia starts to actually respond.
...
- **Response streaming.** Stream tokens back to the client so Navia's reply appears as she "thinks".

**Client-side pieces:**

- Reactive `ConversationStore` that talks to the API.
- Streaming text renderer in the diary so Navia's reply types in, exactly like the landing page hero demo.
...
- Error handling that is kind, not technical. "Navia is catching her breath. One moment."
...
**Milestone:** by end of Phase 3, Sebastien can write a diary entry on his iPhone and get a warm, on-brand reply from Navia in under 2 seconds.
...
5. **Custom voice training.** Send recordings to a provider that supports voice cloning under strict consent, with exclusive rights assigned to Navia by contract.
...
**Milestone:** Sebastien plays Navia's reply on his iPhone, closes his eyes, and feels a real person talking to him.
...
5. Do they say, in their own words, that Navia made them feel less alone?
...
| Tier | Price | Includes |
|------|-------|----------|
| Navia Free | $0 | Check-in, diary, cycle, pregnancy tracking, unlimited companion turns |
| Navia Plus | $9.99/mo | Pattern insights, longer memory, priority voice |
| Navia Care | $29.99/mo | Everything + one curated gift per month |
| Navia Love | $49.99/mo | Everything + two gifts per month + priority companion |
...
Navia handles some of the most sensitive personal data a woman can share. The legal foundation is not optional.

1. **Navia is not a medical device** and will not pursue HIPAA compliance. We will follow best practices anyway: encryption at rest and in transit, zero third-party data sharing, clear retention and deletion policies.
...
3. **Terms of service** must include: "Navia is not a medical device or a substitute for professional care", limitation of liability, minimum user age.
...
6. **Trademark** must be resolved with a US IP attorney before Phase 5. "Navia Benefit Solutions" already exists in the adjacent health benefits space.
...
| Mid September 2026 | Phase 3 closes | Navia replies to diary entries with v1 prompt through our API |
| End of September 2026 | Phase 4 closes | Custom Navia voice live in the app |
...
- **Sebastien** brings the vision, the women's health expertise, Navia's tone, the emotional compass, the voice casting decisions, and the relationships with beta users.
...
- **Not a replacement for the foundation documents.** `personality.md`, `system-prompt-v1.md`, `architecture.md`, and `mvp-features.md` remain the source of truth for what Navia is, how she speaks, and how she is built.
