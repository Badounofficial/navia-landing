...
function DiaryScreen({ colors }) {
  ...
  const [moonDetached, setMoonDetached] = useState(false);
  const [moonArrived, setMoonArrived] = useState(false);
  ...
  const handleSend = () => {
    ...
    // Pick the response now, store it for later
    const responses = [
    ...
    // Phase 1: Moon detaches from the "ı" and travels down (dots visible)
    setTimeout(() => {
      setMoonDetached(true);
      setWritingResponse(true);
    }, 400);

    // Phase 2: Moon has landed (2s animation) — small pause, then quill starts
    setTimeout(() => {
      setMoonArrived(true);
    }, 2400); // 400ms wait + 2s descent animation

    // Phase 3: Text starts writing after moon has settled
    setTimeout(() => {
      setWritingResponse(false);
      setLatestOzaiaText(pendingResponseRef.current);
    }, 3000); // 600ms after landing — let the moon breathe
  };

  // When quill writing completes, moon returns to header
  const handleOzaiaComplete = useCallback(() => {
    setMoonReturning(true);
    setTimeout(() => {
      setMoonDetached(false);
      setMoonArrived(false);
      setMoonReturning(false);
      // Fold quill response into entry
      setEntries((prev) => {
        const updated = [...prev];
        const last = updated[updated.length - 1];
        if (last && !last.ozaia) {
          updated[updated.length - 1] = { ...last, ozaia: latestOzaiaText };
        }
        return updated;
      });
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
      {/* Traveling moon: detaches from header, goes to writing area */}
      {moonDetached && (
        <div
          style={{
            position: "absolute",
            ...
            pointerEvents: "none",
            animation: moonReturning
              ? "moonAscend 0.8s ease-in forwards"
              : moonArrived
                ? "moonFloat 2s ease-in-out infinite"
                : "moonDescend 2s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards",
...
function MoonPhaseIcon({ size = 42, phase = 0.5, colors }) {
  ...
  useEffect(() => {
    ...
    // We paint shadow over the lit surface. The lit crescent stays visible on one side.
    const illum = Math.max(0, Math.min(1, phase));

    if (illum < 0.98) {
      // Method: draw shadow as the FULL circle, then SUBTRACT the lit crescent.
      // Lit crescent = intersection of two circles offset horizontally.
      // Simpler approach: paint shadow on right half + elliptical terminator extending left.

      // For illum 0..0.5: lit crescent on LEFT, shadow covers right + most of center
      // For illum 0.5..1: lit crescent on RIGHT, shadow covers left + some center

      // Terminator ellipse radiusX: 0 at half moon, r at new/full
      const tRx = Math.abs(illum - 0.5) * 2 * r;
      ...
      if (illum <= 0.5) {
        // Shadow covers from RIGHT edge to the terminator (which is left of center)
        // The more left the terminator, the more shadow. At illum=0, shadow=entire circle.
        // Draw: fill entire right half, then fill the area between center and terminator on left
        ctx.beginPath();
        // Right semicircle
        ctx.moveTo(cx, cy - r);
        ctx.arc(cx, cy, r, -Math.PI / 2, Math.PI / 2, false);
        // Curved terminator back to top (ellipse bulges LEFT for low illum)
        ctx.ellipse(cx, cy, tRx, r, 0, Math.PI / 2, -Math.PI / 2, true);
        ctx.closePath();
      } else {
        // Shadow on LEFT side, terminator bulges right
        ctx.beginPath();
        ctx.moveTo(cx, cy - r);
      ...
      // Soft glow at terminator edge
      ctx.beginPath();
      const glowRx = Math.max(2, tRx + 2);
      if (illum <= 0.5) {
        ctx.ellipse(cx, cy, glowRx, r, 0, -Math.PI / 2, Math.PI / 2, false);
      } else {
        ctx.ellipse(cx, cy, glowRx, r, 0, -Math.PI / 2, Math.PI / 2, true);
      }
      ctx.lineWidth = 3;
      ctx.strokeStyle = `rgba(${isNight ? "200,195,180" : "180,175,165"},0.12)`;
      ctx.stroke();
...
/* ── Profile Screen ── */
function ProfileScreen({ colors, theme, onToggleTheme }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 24px" }}>
      ...
      >
        ...
        <div style={{ width: 36, height: 20, borderRadius: 10, background: colors.accentAube, position: "relative", cursor: "pointer", opacity: 0.7 }}>
        ...
        <PhoneFrame colors={colors}>
          {screen === "onboarding" ? (
            ...
            <>
              ...
              {activeTab === "profile" && <ProfileScreen colors={colors} theme={theme} onToggleTheme={setTheme} />}
